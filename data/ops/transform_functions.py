...
def convert2transformation(rotation_matrix: torch.Tensor, translation_vector: torch.Tensor):
    one_ = torch.tensor([[[0.0, 0.0, 0.0, 1.0]]]).repeat(
        rotation_matrix.shape[0], 1, 1).to(rotation_matrix)    # (Bx1x4)
    transformation_matrix = torch.cat(
        [rotation_matrix, translation_vector.unsqueeze(-1)], dim=2)                        # (Bx3x4)
    # (Bx4x4)
    transformation_matrix = torch.cat([transformation_matrix, one_], dim=1)
...
class PNLKTransform:
    ...
    def generate_transform(self):
        ...
        x[:3] = x[:3] / x[:3].norm(p=2, dim=1, keepdim=True)
        x[3:] = x[3:] / x[3:].norm(p=2, dim=1, keepdim=True)
        x[:3] = x[:3] * amp
    ...
    def transform(self, tensor):
        x, rot = self.generate_transform()
        return self.apply_transform(tensor, x), rot
...
class RPMNetTransform:
    ...
    def transform(self, tensor):
        x, theta = self.generate_transform()
        return self.apply_transform(tensor, x), theta

    def __call__(self, tensor):
        return self.transform(tensor)


class SampleNetTransform:
    def __init__(self, max_rotation=45):
        self.max_rotation = max_rotation

    def generate_transform(self):
        axis = np.random.randn(1, 3)
        axis = axis/np.sqrt(np.mean(np.square(axis)))
        angle = 2*self.max_rotation * \
            torch.rand(1).numpy()[0] - self.max_rotation
        angle = angle*(np.pi/180)
        print(angle)
        translation = np.random.rand(3)

        self.igt = np.eye(4)
        self.igt[0:3, 0:3] = transforms3d.axangles.axangle2mat(axis[0], angle)
        self.igt[0:3, 3] = translation
        self.igt = torch.tensor(self.igt).float()

    def apply_transform(self, p0):
        p1 = torch.matmul(self.igt[0:3, 0:3], p0.permute(
            1, 0)).permute(1, 0) + self.igt[0:3, 3]
        return p1
...
class PCRNetTransform:
    def __init__(self, data_size, angle_range=45, translation_range=1):
        ...
        self.transformations = [self.create_random_transform(
            torch.float32, self.angle_range, self.translation_range) for _ in range(data_size)]
    ...
    def quaternion_rotate(point_cloud: torch.Tensor, pose_7d: torch.Tensor):
        ...
        if ndim == 2:
            ...
            quat = PCRNetTransform.get_quaternion(pose_7d).unsqueeze(
                1).expand([-1, N, -1]).contiguous()
            rotated_point_cloud = quaternion.qrot(quat, point_cloud)
    ...
    def quaternion_transform(point_cloud: torch.Tensor, pose_7d: torch.Tensor):
        transformed_point_cloud = PCRNetTransform.quaternion_rotate(point_cloud, pose_7d) + PCRNetTransform.get_translation(
    ...
    def convert2transformation(rotation_matrix: torch.Tensor, translation_vector: torch.Tensor):
        one_ = torch.tensor([[[0.0, 0.0, 0.0, 1.0]]]).repeat(
            rotation_matrix.shape[0], 1, 1).to(rotation_matrix)    # (Bx1x4)
        transformation_matrix = torch.cat(
            [rotation_matrix, translation_vector[:, 0, :].unsqueeze(-1)], dim=2)                        # (Bx3x4)
        # (Bx4x4)
        transformation_matrix = torch.cat([transformation_matrix, one_], dim=1)
        return transformation_matrix
...
class DCPTransform:
    ...
    def generate_transform(self):
        ...
        self.igt = np.concatenate(
            [self.R_ab, self.translation_ab.reshape(-1, 1)], axis=1)
        self.igt = np.concatenate([self.igt, last_row], axis=0)

    def apply_transformation(self, template):
        rotation_ab = Rotation.from_euler(
            'zyx', [self.anglez, self.angley, self.anglex])
        source = rotation_ab.apply(template.T).T + \
            np.expand_dims(self.translation_ab, axis=1)
        return source
